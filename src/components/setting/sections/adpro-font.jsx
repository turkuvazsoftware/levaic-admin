    ...
    return (
        <Fragment>
            <div className="d-flex justify-content-between">
                <h6 className="d-inline-block">Yazı Tipi</h6>
            ...
            <div className="d-grid gap-3 grid-cols-2 mb-4">
                <div>
                    <div className="d-flex justify-content-between align-items-center mb-2">
                        <div className="label">
                            <label className="mb-0 d-inline-block">İçerik Yazı Tipi</label>
                        </div>
                    </div>
                    <div className="form-group mb-0 custom-choicejs">
                        <select className="form-control" onChange={(e) => dispatch(body_font_family(e.target.value))} defaultValue={props.bodyFontFamily !== null ? props.bodyFontFamily : 'Inter'}>
                        {options.map((item, index) => (
                            <option key={index} defaultValue={item.value}>{item.label}</option>
                        ))}
                        </select>
                    </div>
                </div>
                <div>
                    <div className="d-flex justify-content-between align-items-center mb-2">
                        <div className="label">
                            <label className="mb-0 d-inline-block">Başlık Yazı Tipi</label>
                        </div>
                    </div>
                    <div className="form-group mb-0 custom-choicejs">
                        <select className="form-control" onChange={(e) => dispatch(heading_font_family(e.target.value))} defaultValue={props.headingFontFamily !== null ? props.headingFontFamily : 'Inter'}>
                        {options.map((item, index) => (
                            <option key={index} defaultValue={item.value}>{item.label}</option>
                        ))}
                        </select>
